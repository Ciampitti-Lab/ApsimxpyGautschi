...
folder = "/depot/ciampitti/data/WorkflowApsimNitrogenData/_3AgroDataExtractionData"
geojson_file = glob.glob(os.path.join(folder, "*.geojson"))
fields=gpd.read_file(geojson_file[0])


rng = np.random.default_rng(42) 
nitrogen = [100, 125, 150, 175, 200, 225, 250, 275, 300]
# Preproccesing
soils=pd.read_csv("/depot/ciampitti/data/WorkflowApsimNitrogenData/_4RunSimulationsData/soil/soils.csv")
cols = ['SAND', 'CLAY', 'SILT', 'BD']
soils = soils[~(soils[cols].fillna(0) == 0).all(axis=1)]

lis_df=[]
count=0
apsim_files="/depot/ciampitti/data/WorkflowApsimNitrogenData/_4RunSimulationsData/apsim_files"
if os.path.exists(apsim_files) and os.path.isdir(apsim_files):
    shutil.rmtree(apsim_files)
...
# for id,row in sample_fields[sample_fields['id_cell']>=1167].iterrows():
for id,row in sample_fields.iterrows():
    ...
    if row['id_within_cell']%2==0:
        for year in list(range(2006,2024,2)):
            
            init_obg=apsimxpy.Initialize(apsim_folder_input='/home/jjolaher/WorkflowApsimNitrogenRCAC/WorkflowApsimNitrogenIndiana/ApsimxpyGautschi',apsim_file_input= apsim_file)
            ...
            for n_rate in nitrogen:
                ...
                shutil.copy(f"/home/jjolaher/WorkflowApsimNitrogenRCAC/WorkflowApsimNitrogenIndiana/ApsimxpyGautschi/{apsim_file}.apsimx",f"/depot/ciampitti/data/WorkflowApsimNitrogenData/_4RunSimulationsData/apsim_files/{apsim_file}_{row['id_cell']}_{row['id_within_cell']}_N{n_rate}_Y{year}.apsimx")
                # Counting simulations
                count+=1
    else:
        for year in list(range(2007,2024,2)):
            init_obg=apsimxpy.Initialize(apsim_folder_input='/home/jjolaher/WorkflowApsimNitrogenRCAC/WorkflowApsimNitrogenIndiana/ApsimxpyGautschi',apsim_file_input= apsim_file)
...
# # tasks = [row.to_dict() for _, row in sample_fields[sample_fields['id_cell']>=1167].iterrows()] 
